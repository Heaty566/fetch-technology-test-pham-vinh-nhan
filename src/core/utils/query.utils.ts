...
import { PagingFilter, pagingFilterValidationSchema } from '../dtos';
import { CompareOperator } from '../repositories';

export const queryGenerator = <
    T extends Record<string | keyof PagingFilter, any>,
>(
    input: T,
    comparator: Record<
    ...
    input = Object.keys(input).reduce((acc, key) => {
        if (
            ...
            input[key] !== null &&
            input[key] !== '' &&
            !Object.keys(pagingFilterValidationSchema).includes(key)
        ) {
